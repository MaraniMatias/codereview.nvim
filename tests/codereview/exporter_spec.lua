...
describe("review.exporter.generate() — default format", function()
  ...
  it("shows smart range {5} in fence when start==end", function()
    store.set("a.lua", 5, 5, "", "single line", "new")
    local out = exporter.generate()
    assert.truthy(out:find("```lua{5}", 1, true))
    assert.falsy(out:find("```lua{5,5}", 1, true))
  end)

  it("shows range {7,9} in fence when start!=end", function()
    store.set("a.lua", 7, 9, "local x = 1\nlocal y = 2", "check math", "new")
    local out = exporter.generate()
    assert.truthy(out:find("```lua{7,9}", 1, true))
  end)

  it("marks old-side notes with minus prefix in fence range", function()
    store.set("src/bar.lua", 10, 10, "", "deleted note", "old")
    local out = exporter.generate()
    assert.truthy(out:find("```lua{-10}", 1, true))
  end)

  it("marks old-side range notes with minus prefix on both numbers", function()
    store.set("src/bar.lua", 10, 15, "", "deleted range", "old")
    local out = exporter.generate()
    assert.truthy(out:find("```lua{-10,-15}", 1, true))
  end)

  it("uses language-specific code block fencing with line info", function()
    store.set("app.js", 1, 1, "const x = 1", "check", "new")
    local out = exporter.generate()
    assert.truthy(out:find("```js{1}", 1, true))
  end)
  ...
  it("includes code block with line info when code is provided", function()
    store.set("a.lua", 7, 9, "local x = 1\nlocal y = 2", "check math", "new")
    local out = exporter.generate()
    assert.truthy(out:find("```lua{7,9}", 1, true))
  ...
  it("emits empty code block with line info when no code", function()
    store.set("a.lua", 3, 3, "", "missing code note", "old")
    local out = exporter.generate()
    assert.truthy(out:find("```lua{-3}", 1, true))
  end)
  ...
  it("keeps backticks in code inside an adaptive fence", function()
    store.set("a.lua", 1, 1, "print(`one`)\n````", "check", "new")
    local out = exporter.generate()
    assert.truthy(out:find("`````lua{1}", 1, true))
  end)

  it("quotes note Markdown so it cannot escape the report", function()
    store.set("a.lua", 1, 1, "code", "# heading\n```\n---", "new")
    local out = exporter.generate()
    assert.truthy(out:find("> # heading", 1, true))
    assert.truthy(out:find("> \\`\\`\\`", 1, true))
  end)
end)
...
describe("review.exporter.generate() — table format", function()
  ...
  it("starts with the documented table header", function()
    local out = exporter.generate()
    assert.truthy(out:match("^file|line|side|text"))
  end)
  ...
  it("includes pipe-separated fields for a note", function()
    ...
    local out = exporter.generate()
    assert.truthy(out:find("src/foo.lua|5-8|new|some note", 1, true))
  end)

  it("uses smart range (no redundant 5-5)", function()
    store.set("a.lua", 5, 5, "", "single line", "new")
    local out = exporter.generate()
    assert.truthy(out:find("a.lua|5|new|", 1, true))
  ...
  it("shows range when start!=end", function()
    ...
    local out = exporter.generate()
    assert.truthy(out:find("a.lua|5-10|new|", 1, true))
  end)

  it("includes old-side notes in the side field", function()
    store.set("src/bar.lua", 10, 12, "", "old note", "old")
    local out = exporter.generate()
    assert.truthy(out:find("src/bar.lua|10-12|old|old note", 1, true))
  end)
  ...
  it("does not add (del) suffix for new-side notes", function()
    ...
    assert.truthy(out:find("src/bar.lua|10-12|new|new note", 1, true))
  end)

  it("escapes multiline notes without creating extra table rows", function()
    store.set("a.lua", 1, 1, "", "line one\nline two", "new")
    local out = exporter.generate()
    assert.truthy(out:find("line one\\nline two", 1, true))
  ...
  it("includes all notes from all files", function()
    ...
    local out = exporter.generate()
    assert.truthy(out:find("a.lua|1|new|note a",   1, true))
    assert.truthy(out:find("b.lua|3-5|new|note b", 1, true))
  end)

  it("escapes table delimiters and line breaks", function()
    store.set("a|b.lua", 1, 1, "", "left|right\nnext", "new")
    local out = exporter.generate()
    assert.truthy(out:find("a\\|b.lua|1|new|left\\|right\\nnext", 1, true))
  end)
end)
